...
from screen_utils import get_window_name, get_pixels
from IConfigurable import IConfigurable
from config import ScriptConfig
from color_math import is_blue_hsv, is_pink_color, is_red_color, is_green_color, is_blue_color
    ...
    def get_game_state_for_pixel_colors(
        ...
        if self.config.configData.chaos_inoculation:
            ...
            if game_state.is_poison:
                game_state.is_energy_shield_below_threshold = not any(is_pink_color(color) for color in energy_shield_pixels_rgb)
            else:
                game_state.is_energy_shield_below_threshold = not any(is_blue_color(color) for color in energy_shield_pixels_rgb)
        else:
            game_state.is_energy_shield_below_threshold = not any(is_blue_color(color) for color in energy_shield_pixels_rgb)
            game_state.is_poison = is_green_color(poison_pixel_rgb)
            if game_state.is_poison:
                game_state.is_health_below_threshold = not any(is_green_color(color) for color in health_pixels_rgb)
            else:
                game_state.is_health_below_threshold = not any(is_red_color(color) for color in health_pixels_rgb)
