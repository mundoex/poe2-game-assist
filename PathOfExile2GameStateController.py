...
class PathOfExile2GameStateController():
    ...
    def check_game_state(self) -> None:
        ...
        self.update(new_game_state)

    def update(self, new_game_state: PathOfExile2GameState) -> None:
        self.state = new_game_state
        for state_attr, event_name in STATE_ATTR_TO_EVENT.items():
            new_value = getattr(self.state, state_attr)
            if new_value:
                self.emit_if_not_on_cooldown(event_name, new_value)

        self.emit_if_not_on_cooldown("on_repeat", True)

    def emit_if_not_on_cooldown(self, event_name: str, value: bool) -> None:
        ...
        if not on_cooldown:
            ...
            self.events.emit(event_name, value)